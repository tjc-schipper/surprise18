...
public class Draggable : MonoBehaviour
{
	...
	public delegate void SnapEvent(Draggable sender, Transform snapLocation);
	public event SnapEvent OnSnap;
	public event SnapEvent AbortDragging;
	...
	private void LateUpdate()
	{
		...
		if (this.dragging)
		{
			...
			RaycastHit hit;
			Vector3 viewspaceMousePos = Input.mousePosition;
			viewspaceMousePos.z = this.zDistance;
			this.currentRay = new Ray(Camera.main.transform.position, Camera.main.ScreenToWorldPoint(viewspaceMousePos) - Camera.main.transform.position);

			if (Physics.Raycast(this.currentRay, out hit, 500f, LayerMask.GetMask("SnapCollider")))
			{
	...
	private void Clickable_MouseUp(Clickable sender, UnityEngine.EventSystems.PointerEventData e)
	{
		if (this.dragging)
		{
			...
			if (this.activeConstraints != null && this.activeConstraints[1] != null)
			{
				...
				if (this.OnSnap != null)
					this.OnSnap.Invoke(this, this.activeConstraints[1].transform);
			}
			else
			{
				this.snapLocation = this.originalLocation;
				if (this.AbortDragging != null)
					this.AbortDragging.Invoke(this, null);
			}
		}
	}
